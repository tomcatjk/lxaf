...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List findRoles(User user){
		return roleDao.getRoles(user.getCustomerID());
	}

	public String findRoleId(User user){
		return roleDao.getRoleId(user.getId());
	}

	public Page<User> findUserPage(Page<User> page, User user) {
		user.setPage(page);
		page.setList(userDao.getUserPage(user));
		for(User userTemp : page.getList()){
			userTemp.setCustomerID(userDao.finUserCustomers(userTemp.getCustomerID()));
			userTemp.setLoginFlag(LoginTypeName.getByType(Integer.parseInt(userTemp.getLoginFlag())).getLoginTypeName());
