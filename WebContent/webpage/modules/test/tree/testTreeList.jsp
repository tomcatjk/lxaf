...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			/*字符串 stringObject ($("#treeTableTpl").html())的 replace() 方法执行的是查找并替换的操作。它将在 stringObject 中查找与 regexp((\/\/\<!\-\-)|(\/\/\-\->))
			相匹配的子字符串，然后用 replacement ("")来替换这些子串。如果 regexp 具有全局标志 g，那么 replace() 方法将替换所有匹配的子串。否则，它只替换第一个匹配子串。*/
			var tpl = $("#treeTableTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
			var data = ${fns:toJson(list)}, ids = [], rootIds = [];
			for (var i=0; i<data.length; i++){
				ids.push(data[i].id);
			}
			/*join(','):在ids中的每一个数中间加一个,号*/
			ids = ',' + ids.join(',') + ',';
			for (var i=0; i<data.length; i++){
...
<body class="gray-bg">
	...
	<table id="treeTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<%--标签里面的内容不会被执行，也不会显示在页面上，但是可以在另一个script里面通过获取，然后再通过相关的模板工具进行处理，插入到页面中。这样就把大段的HTML操作从js里面分离开--%>
