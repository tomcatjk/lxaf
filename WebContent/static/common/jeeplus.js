	    ...
	    yes: function(index, layero){
	        ...
	        if(iframeWin.contentWindow.doSubmit() ){
	        	// top.layer.close(index);//关闭对话框。
	        	  setTimeout(function(){top.layer.close(index);submitFrom();}, 100);//延时0.1秒，对应360 7.1版本bug
	         }
		  },
...
//打开对话框(修改)customersForm.jsp -------- khb add ---
function openDialogEdit(title,url,width,height,target){
	...
	top.layer.open({
		...
		yes: function(index, layero){
			...
			if(iframeWin.contentWindow.saveCustomer()){
				// top.layer.close(index);//关闭对话框。
				setTimeout(function(){
					window.location = ctx + "/lu/customers/?customertype=" + customertype;
					top.layer.close(index);
...
//打开对话框(添加)customersForm.jsp -------- khb add ---
function openDialogAdd(title,url,width,height,target){
	...
	top.layer.open({
		...
		yes: function(index, layero){
			var body = top.layer.getChildFrame('body', index);
			var iframeWin = layero.find('iframe')[0]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
			var inputForm = body.find('#reg-form');
			var top_iframe;
			if(target){
				top_iframe = target;//如果指定了iframe，则在改frame中跳转
			}else{
			...
			iframeWin.contentWindow.showTab();
			var showTabId = iframeWin.contentWindow.showTabId;
			if(showTabId == 0){
				setTimeout(function(){
					top.layer.close(index);
					// sortOrRefresh();
